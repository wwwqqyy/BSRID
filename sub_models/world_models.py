...
class DecoderBN(nn.Module):
    def __init__(self, stoch_dim, last_channels, original_in_channels, stem_channels, final_feature_width) -> None:
        ...
        backbone = [nn.Linear(stoch_dim, last_channels * final_feature_width * final_feature_width, bias=False),
                    Rearrange('B L (C H W) -> (B L) C H W', C=last_channels, H=final_feature_width),
                    nn.BatchNorm2d(last_channels), nn.ReLU(inplace=True)]
        # stem
        # residual_layer
...
class WorldModel(nn.Module):
    ...
    def init_imagine_buffer(self, imagine_batch_size, imagine_batch_length, dtype):
        ...
        if self.imagine_batch_size != imagine_batch_size or self.imagine_batch_length != imagine_batch_length:
            print(f"\ninit_imagine_buffer: {imagine_batch_size}x{imagine_batch_length}@{dtype}")
    ...
    def imagine_data(self, agent: agents.ActorCriticAgent, sample_obs, sample_action,
        ...
        # context
        context_latent = self.encode_obs(sample_obs)  # 1024*8*1024
        # print('context_latent.shape', context_latent.shape) torch.Size([1024, 8, 1024])
        # print('context_latent[:, i:i + 1].shape', context_latent[:, 0:1].shape) torch.Size([1024, 1, 1024])
        ...
        if log_video:
            logger.log("Imagine/predict_video",
                       torch.clamp(torch.cat(obs_hat_list, dim=1), 0, 1).cpu().float().detach().numpy())

        return torch.cat([self.latent_buffer, self.hidden_buffer],
                         dim=-1), self.action_buffer, self.reward_hat_buffer, self.termination_hat_buffer
